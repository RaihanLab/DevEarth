  ...
  buildProceduralMaterial() {
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        vec3 landColor(float h, float moisture, float lat) {
        ...
        void main() {
          ...
          if (isOcean) {
            ...
            baseColor = mix(vec3(0.08,0.28,0.48),mix(vec3(0.05,0.20,0.40),vec3(0.02,0.08,0.22),depth),depth);
          } else {
            baseColor = landColor(height,moisture,lat);
          }
          vec3 normal=normalize(vNormal),sun=normalize(sunDirection);
          float sunDot=dot(normal,sun);
          float dayNight=smoothstep(-0.18,0.28,sunDot);
          vec3 dayLit=baseColor*(0.10+max(0.0,sunDot)*1.15);
          vec3 nightColor=isOcean?vec3(0.0):vec3(0.95,0.80,0.30)*0.12;
          if (isOcean) {
            vec3 viewDir=normalize(cameraPosition-vWorldPos);
            vec3 halfVec=normalize(sun+viewDir);
