...
function animate() {
  ...
  if (autoRotate && earth.earthMesh) {
    earth.earthMesh.rotation.y += 0.0008;
    if (earth.cloudMesh) earth.cloudMesh.rotation.y += 0.00085;
    if (earth.atmosphereMesh) earth.atmosphereMesh.rotation.y += 0.0008;
    if (earth.glowMesh) earth.glowMesh.rotation.y += 0.0008;
  }

  if (!isDragging && earth.earthMesh) {
    if (Math.abs(rotationVelocity.y) > 0.0001 || Math.abs(rotationVelocity.x) > 0.0001) {
      earth.earthMesh.rotation.y += rotationVelocity.y;
      earth.earthMesh.rotation.x += rotationVelocity.x;
      if (earth.cloudMesh) {
        earth.cloudMesh.rotation.y += rotationVelocity.y;
        earth.cloudMesh.rotation.x += rotationVelocity.x;
      }
      if (earth.atmosphereMesh) {
        earth.atmosphereMesh.rotation.y += rotationVelocity.y;
        earth.atmosphereMesh.rotation.x += rotationVelocity.x;
      }
